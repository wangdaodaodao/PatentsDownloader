...
def get_pdf(patent_no='CN201510708735.4'):
    ...
    if '错误' in response_search.text:
        ...
        #     pass
        file_name = dir_path + os.sep + '{name}-{number}.pdf'
        down_file(file_url.format(url=host_name, numbers=patent_no), file_name.format(name=urllib.parse.unquote(data_securepdf.get('Name')), number=patent_no))
